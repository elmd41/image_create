...
### v1.0.1 (2026-03-13)
- ✅ 套色默认保持原图分辨率，不再强制输出正方形
- ✅ 套色结果图改为等间距自适应排布，视觉更平衡
- ✅ 历史会话切换增加前端缓存与请求中止，切换更快
- ✅ 上传前格式转换与压缩策略增强（生产/开发环境统一）
...
### 1) 套色后图片变形或变成正方形怎么办？
- 当前版本默认按原图尺寸进行套色生成。
- 如需验证，可对比原图和结果图像素尺寸，应保持一致。

### 2) 切换历史会话感觉慢？
- 已加入会话消息缓存，切换时优先读缓存并后台刷新。
- 同时中止旧会话请求，避免慢请求覆盖当前页面。

### 3) 为什么有些大图上传失败？
- 上传前会自动执行：格式检测 → 必要时后端转换 → 多轮压缩/降采样。
- 建议优先使用 PNG/JPG/WEBP；TIFF/BMP/HEIC 会自动转换后再上传。
- ✅ 多图生成支持
- ✅ 取消生成功能
